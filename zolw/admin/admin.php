...
function listaKategorii($link) {
    ...
    $result = $link->query("SELECT id, nazwa FROM kategoria LIMIT 10");
    while ($row = $result->fetch_array()) {
        $id = htmlspecialchars($row['id']);
        ...
        echo "<tr>
                ...
                <td>{$row['nazwa']}</td>
                <td>
                    <a href='?edit_id={$row['id']}&type=category'><b>Edytuj</b></a> <b>|</b>
                    <a href='?delete_id={$row['id']}&type=category'><b>Usuń</b></a>
                </td>
              </tr>";
    }
    echo "</table>";
...
function listaProduktow($link) {
    ...
    $result = $link->query("SELECT * FROM products LIMIT 10");
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
                ...
                <td>{$size}</td>
                <td><img src='data:image/jpeg;base64," . base64_encode($image) . "' alt='Product Image' width='50' height='50'></td>
                <td>
...
function DodajNowyProdukt($link) {
    ...
    echo "<form method='post' enctype='multipart/form-data'>
        ...
        <label for='image'><b>Obrazek:</b></label><br>
    ...
    if (isset($_POST['save_new_product'])) {
        ...
        //  zapisanie obrazka jako BLOB jesli dostał dodany
        if ($_FILES['image']['tmp_name']) {
            $image = file_get_contents($_FILES['image']['tmp_name']);
        } else {
            $image = null; // Jeśli brak obrazka, ustawia na null
        }
...
function EdytujProdukt($id, $link) {
    ...
    if (isset($_POST['save_changes'])) {
        ...
        $stmt->bind_param("ssssisisssi", $new_title, $new_description, $new_creation_date, $new_expiration_date, $new_net_price, $new_vat_tax, $new_available_quantity, $new_availability_status, $new_size, $new_image, $id_clear);
